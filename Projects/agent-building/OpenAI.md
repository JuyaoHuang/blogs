...
OpenAI 的发送格式不是一个单一的字符串输入，而是一个消息列表，即 python 的 List 格式。列表里的每一个元素是字典 dict。
...
每个模型都有记忆上限，如果一直聊下去，总有一刻会爆显存。OpenAI 的设计目的之一就是让用户能够控制历史记忆（上下文窗口，message）
...
OpenAI 格式的**设计目的**就是用最低的服务器成本，换取用户对对话上下文（Context）和指令权限的最高控制度。

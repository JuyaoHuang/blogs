...
1. 当历史对话到达一定规模，例如上下文窗口的 70%，将对话历史中较早的部分，例如前一半，提取出来，使用**独立的 API 请求**调用大模型进行摘要和总结
...
- **客户端渲染**：在 Web 前端，使用 `ReadableStream` 和 `TextDecoderStream` API 可以平滑地处理和渲染 SSE 事件流，提供最佳的用户体验

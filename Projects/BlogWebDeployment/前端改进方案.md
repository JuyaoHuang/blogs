...
<div align="right"> <a href="./frontedImproved">English</a> | <a href="./前端改进方案"><strong>简体中文</strong></a> </div>
...
2. **卡片式呈现文章**
   1. 取消目前呈现的"点击分类后以时间线 Archive 呈现文章的形式"，修改为类似主页呈现文章的卡片式页面	
...
2. **组件接口适配**：
   ...
   - **PostCard 组件** (`src/components/PostCard.astro`)：
     - 更新 Props 接口支持二级分类字段
   ...
   - **数据传递优化** (`src/utils/content.ts`)：
     ...
     - 统一数据传递格式：传递原始 ID，由组件统一处理 URL 生成
     - 确保 HASH/RAW 模式兼容性
...
3. **URL 生成策略**：
   ...
   - 支持 HASH 和 RAW 两种 slug 模式
   - 统一使用 `IdToSlug()` 函数处理 URL 转换
   - 解决 URL 重复前缀问题，避免 404 错误
...
3. **用户体验提升**：
   ...
   - 分类信息正确显示在 PostCard 组件中
   - 点击分类标签导航到对应分类页面
   - 最新文章优先显示，无需下滑查找
   - URL 结构清晰，支持直接访问和书签保存
...
3. **URL 生成优化**：
   - ✅ 统一数据传递架构：原始 ID 传递，组件统一处理
   - ✅ 模式无关设计：代码支持 HASH/RAW 模式无缝切换
   - ✅ 404 问题修复：解决 URL 重复前缀导致的路由错误

4. **用户体验提升**：
   - ✅ 文章排序：分类页面按发布日期降序排列（最新在前）
   - ✅ 分类信息显示：PostCard 组件正确显示一级和二级分类
...
2. **关键函数**：
   ...
   - 统一的 URL 生成逻辑：`IdToSlug()` 根据配置模式处理
...
3. URL 自动更新（RAW 模式直观可读，HASH 模式生成新 hash）
...
1. **数据提取**
   ...
   - text: 标题的纯文本内容。
   - slug: 根据标题文本生成的、URL 友好的唯一 ID，Astro 会自动将其作为 id 属性注入到对应的` <h2>` 等标签中。
...
1. 获取目录数据 
   ...
   - 包含：depth(级别)、text(文本)、slug(锚点 ID)

2. 传递给浮动 UI 组件 
   - 使用 Astro 的数据驱动架构
   - 将 headings 数组直接传递给 Svelte 组件
   - 完全解耦，不影响文章内容

3. 文章内容保持不变 
   - Astro 自动为标题生成 id 属性（如` <h2 id="hello-world">`）
   - 浏览器原生支持锚点跳转` #hello-world`
   - 无需修改 markdown 源文件

####  技术优势

  比之前的 mdast-util-toc 方案更优：
...
#### 实现步骤（简化版）
    ...
    1. 修改文章页面：获取 Astro 内置的 headings 数据
    2. 创建 Svelte 组件：接收 headings 数组，渲染浮动 TOC
    3. 添加交互功能：滚动高亮、平滑跳转

  关键点：不需要任何 remark 插件，完全基于 Astro 原生功能
...
4. **精细的 UI/UX 设计**:
   - **定制化主题**：采用 muzimi 颜色主题，并应用毛玻璃效果 (backdrop-filter) 提升视觉质感。
...
​	因为 FloatingTOC 组件使用了 client:load 指令，在 Astro 的页面间导航时，JavaScript 组件实例可能不会重新初始化。项目使用了 Swup 进行页面间的 SPA 导航，导致 Svelte 组件无法正确响应页面变化。
...
2. 集成到 Swup 的 `content:replace` 钩子中，确保每次页面切换都重新创建 TOC
3. 添加旧组件清理逻辑，防止内存泄漏
...
1. **缺少 Sharp 依赖**：Astro 构建过程中因为缺少 `sharp` 图像处理库导致构建失败，pagefind 索引无法生成
...
- **开发模式**：`http://localhost:4323/` 运行无错误，使用 mock 搜索对象
- **构建模式**：`[pagefind] Pagefind indexed 27 pages` 成功生成索引
- **预览模式**：`http://localhost:4322/` 完整搜索功能正常工作

**技术要点总结**

1. **SPA 导航兼容性**：使用 Swup 的项目中，需要将组件逻辑集成到页面切换钩子中
2. **资源依赖管理**：确保构建时依赖完整，特别是图像处理相关的 Sharp 库

...
我们来用一种更概念化、更具比喻性的方式来介绍 PyTorch 的核心基础模块
可以把搭建和训练一个神经网络想象成组装和启动一台复杂的机器

### 1. torch.Tensor：核心构件与原材料

这是 PyTorch 世界的**原子**，即 **张量**
...
这是 PyTorch 的核心物理引擎：**自动求导**
它赋予了 Tensor 记忆和反思的能力
...
- **激活函数**
  - 这些是为神经网络注入“非线性”能力的 **"活力催化剂"**，例如 ReLU, Sigmoid, Tanh 等
  - **作用**
...
- **损失函数 Loss Functions**
  ...
  - 这是模型的 **"性能评估器"或"误差计算器"**
    例如 nn.MSELoss (均方误差损失), nn.CrossEntropyLoss (交叉熵损失) 等
  - **作用**
    它负责衡量模型当前的预测结果与真实目标之间的差距
    这个"差距值"（即损失）是整个模型学习和优化的根本依据，损失越大，说明模型表现越差。

### 4. torch.optim：模型的优化器与调校工具

- 这是负责更新和优化模型参数的 **"智能调校引擎"**，它实现了各种优化算法。
- **工作原理**
    1. 它接收模型中所有需要学习的参数，并从 autograd 系统那里获取每个参数的梯度
    2. 然后，它根据一套特定的规则（例如 SGD、Adam、RMSProp 等算法）来微调这些参数，目标是让下一次计算出的损失值变得更小
- **作用**
    将 "计算梯度" 和 "更新参数" 这两个步骤解耦开来，你只需要选择一个合适的 "引擎"（优化器），它就会自动地、迭代地改进你的模型

### 5. torch.utils.data：高效的数据供给系统

这是负责处理和供给数据的 **"后勤部门"**，主要由两个部分组成：
...
- **DataLoader**
  - 这是一个 **“自动化数据传送带”**
  - **作用**
    它从 Dataset 这个 “仓库” 中取出数据，并自动完成一系列繁琐但重要的后勤工作，包括：
...
- **Dataset**
    ...
    负责**将数据封装成标准接口**
  - 它定义了数据有多长 (__len__)，以及如何获取**任何一个**数据样本 (__getitem__)
  - 就像一个 "零件规格书"，告诉流水线工头 DataLoader仓库里有什么，以及怎么取

- **DataLoader**
    负责**高效地、自动化地供给数据**
  - 从 Dataset 中取出数据，并自动打包成 **小批量**
  - 还能**打乱数据顺序 shuffle**，防止模型学到数据的排列规律
  - 还能使用**多进程并行加载**，确保 GPU 在计算时，CPU 已经在后台准备好了下一批数据，防止生产线停工
  - 就是那条高效、智能的自动化传送带

**该阶段 PyTorch 并不关心你的数据 "是什么"，而是关心 "如何高效地喂给模型"**

### 3. 引擎组装与调校：模型构建与训练

**主力工具：`torch.nn`,`torch.optim`,`torch.Tensor`：PyTorch 的核心**

这是 PyTorch **绝对的核心地带**，也是总结的 模型/算法的构建：
...
- **线性层**就像一根笔直的乐高积木
- **没有激活函数的网络**就像把很多根笔直的积木首尾相连——得到的永远是一根更长的、笔直的积木
- **非线性激活函数**就像一个乐高铰链或关节零件
- **带有激活函数的网络**就像在每两根积木之间都加了一个关节。现在，你可以用这些积木和关节搭建出各种复杂的形状、角度和结构，而不仅仅是一条直线
...
这些 logits 的范围可以是任意的负数或正数。输出层激活函数的作用不是为了增加模型的复杂度，而是将这些原始的 logits **转换成一种特定且有意义的格式**，使其与任务的预期输出完全匹配
...
**总结：隐藏层激活函数是为  “过程”  服务的，旨在增强模型能力；输出层激活函数是为“结果”服务的，旨在格式化最终答案**
...
1. **ReLU (Rectified Linear Unit)**
   ...
   **现代神经网络的默认首选，几乎是所有类型的神经网络的默认激活函数**
   ...
   - 缺点-**Dying ReLU Problem** (神经元死亡问题)：
        如果一个神经元的输入在训练过程中始终为负，那么它的输出将永远是 0，梯度也永远是 0
        这个神经元就 "死亡" 了，再也无法通过梯度下降进行更新
   
   - 场景：几乎所有类型的神经网络隐藏层都可以优先尝试使用 ReLU
   
2. **Leaky ReLU 及其变体 (PReLU, ELU)**

   - 优点
        它们是 ReLU 的改进版，旨在解决 "神经元死亡" 问题。当输入为负数时，它们允许一个微小的、非零的梯度通过，使得神经元有机会 "复活"
...
4. **Swish (或 SiLU, Sigmoid-weighted Linear Unit)**

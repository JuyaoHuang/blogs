...
- **深度学习核心概念**：神经网络、卷积神经网络 (CNN)、卷积层、池化层、全连接层、激活函数 (ReLU)。
- **图像分类任务**：CIFAR-10 数据集、多类别分类。
- **深度学习框架**：PyTorch / TensorFlow 的使用。
- **模型训练**：损失函数 (交叉熵)、优化器 (Adam, SGD)、前向传播、反向传播、批处理 (Batch)、轮次 (Epoch)。
- **模型评估与分析**：验证集、混淆矩阵、过拟合与欠拟合。
- **工程实践**：模型保存与加载、使用 TensorBoard 进行可视化。
- **重要细节**：CrossEntropyLoss 与 Softmax 的关系。

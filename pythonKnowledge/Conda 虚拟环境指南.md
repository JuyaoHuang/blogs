...
```
(myenv) Alen@Alen MINGW64 /d/Coding/GitHub_Resuorse/new-llmvtuber/Open-LLM-VTuber (main)  
```
...
**使用 conda install (推荐，Conda 会处理依赖关系)：**
...
在激活你想要导出的环境后，将其导出为 environment.yml （命名你定）件。
...
当你需要复现一个环境时，可以使用导出的 environment.yml 文件来创建它。
...
1. 使用 environment.yml 文件的 name：字段定义新的环境名称：
   ...
   Conda 会读取 environment.yml 文件，并根据文件中的配置创建一个新的环境。新环境的名称将由 environment.yml 文件中的 name: 字段定义。
...
2. 自定义新的环境名称： 
   ...
   - 将 **path/to/your/environment_to_import.yml** 替换为你的 .yml 文件的实际路径。
...
2. 使用 conda env update 命令进行更新：
   ...
   - --prune:    **这是一个非常重要的选项。**   它会删除当前 llmvtuber 环境中存在、但**不包含**在所提供的 YAML 文件中的那些包。这有助于确保你的目标环境与 YAML 文件定义的配置完全一致。如果你不希望删除多余的包，可以省略此选项，Conda 将只添加或更新 YAML 文件中列出的包。
...
5. 重要注意事项
   ...
   1. **YAML 文件内容**： 
      ...
      ​	确保你使用的 .yml 文件来源可靠，并且其内容确实是你想要复制的环境配置。可以先用文本编辑器打开 YAML 文件查看其 name, channels, 和 dependencies 部分。

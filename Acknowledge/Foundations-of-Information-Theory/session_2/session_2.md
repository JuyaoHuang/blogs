...
例如事件 X：小明今天带了伞；事件 Y：今天会下雨。互信息描述的就是 X，Y 直接的关系紧密度。
...
相对熵虽然常被称为 KL 距离，但它**并不是一个真正的距离度量**，因为它不满足距离的两个基本性质：
...
1.  **非对称性**
    ...
    * $D(Q\|P)$ 衡量用 $P$ 近似 $Q$ 的信息损失。
      ...
      这两者通常是不同的。举个例子，如果某个事件在 $P$ 中概率不为 0，但在 $Q$ 中概率为 0，那么 $D(P\|Q)$ 会是无穷大（表示用 $Q$ 来描述 $P$ 是完全错误的）；反之则不一定。
...
D(P\|Q) \ge 0
$$
当且仅当两个分布完全相同时，即 $P(x) = Q(x)$ 对所有 $x$ 成立，相对熵 $D(P\|Q)$ 才为 0。这个性质也称为**吉布斯不等式**。

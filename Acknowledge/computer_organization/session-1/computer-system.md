...
执行一条指令需要花费的时钟周期数。
$$
执行一条指令的耗时 = CPI \times \text{CPU时钟周期}
$$
...
运行一个完整的程序需要花费的时间
$$
CPU 执行时间 = \text{CPU时钟周期数}/主频 = (指令条数 \times CPI)\div主频
$$
...
此处 K、M、G、T等都是数量单位的缩写，例如 $3\times10^6 \text{FLOPS} = 3\times10^3\text{KFLOPS}$
...
### 主频高的 CPU 一定比主频低的CPU快么

未必。假设两个CPU：A 的主频为 2GHz，B为 1GHz，**但是**， A 的 CPI 为 10，B的为1。那么执行一条指令的耗时：A为 5 ，B为 1（CPI÷主频），显然 B 更快

### A、B两个CPU的CPI相同，那么 A 一定快么

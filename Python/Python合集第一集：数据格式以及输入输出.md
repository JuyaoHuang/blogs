...
### 字典： Dictionary------无序记录一堆 Key-Value 的数据集合 ------hash 表
...
1. 第一个方法：占位，也就是类似 C++中的 printf("%d", 123) 此用法
...
2. 第二个方法： Python 中有一个语法 : f"输出内容{变量}"    可以快速地输出结果，不需要使用 %() 插入符

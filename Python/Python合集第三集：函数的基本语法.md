...
# Python 合集第三集：函数的基本语法

## 函数：

 定义：提供已经封装好的一个实现了特定功能代码片段的接口，该代码段可重复使用
 使用 def 去定义函数
 例子 1：使用自己编写的函数，实现对字符串长度的统计
 ...
 **特殊返回值：None**
 **若函数没有写 return 语句，等价于 return None，表示函数体返回值为 None，即"空"。**
l 例如
...
num 就是全局变量，data 为局部变量
若想将 data 变为全局变量，可使用关键字 "**global**" 将其变为全局变量 ---- **不推荐这么做**，毕竟你都在函数体内定义此变量了，为什么还要扩大它的作用域。
